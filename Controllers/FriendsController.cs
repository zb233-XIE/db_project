...
namespace WEBTEST.Controllers
{
    public class FriendsController : Controller
    {
        ...
        public IActionResult GetFriends()
        {
            ...
            friends = friendservice.GetFriends(UID);

            int LENGTH=friends.Count;
            for(int i=0;i<LENGTH;i++)
            {
                FriendModel friendModel = new FriendModel();
                friendModel.ID = friends[i].FriendID;
                Buyers Buyers = _context.Buyers.Where(x => x.BuyerID == friendModel.ID).FirstOrDefault();
                friendModel.Name = Buyers.BuyerName;
                friendModels.Add(friendModel);
            }
            var jsonstr = JsonConvert.SerializeObject(friendModels);
            return Content(jsonstr);
        }
